# Morale feature reference

This is the detailed description of everything Morale can do, including its
limits. For a gentle introduction, start with the [user guide](USER_GUIDE.md).

Menu names are shown in **bold**, for example **File → Digitize artwork**.
...
- **File → Digitize artwork** accepts SVG vectors directly and traces PNG/JPEG/BMP/WebP color regions into
  ...
  **Split suitable branching shapes** cuts Y, X, star and letter-like forks at
  ...
  columns overlap instead of just meeting. Photographic digitizing remains pending. See [image digitizing](IMAGE_DIGITIZING.md).
...
retain the `.morale` file for editable geometry. See [the v1 parity ledger](V1_PARITY.md)
and [the roadmap](ROADMAP.md). V1 parity remains in progress.
