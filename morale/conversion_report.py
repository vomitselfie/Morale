...
def save_conversion_review(path,source_name,info,stitches):
    ...
    stats=info['trace_stats'];text=quality_text(stats['quality'])
...
def _write(path,name,info,source,vector,stitches,maps,text):
    ...
    def page(title):
        ...
        painter.setFont(QFont('Sans',9));painter.drawText(QRectF(margin,height-23,width-2*margin,20),f'Morale · {number}/{pages+3} · Review only; not an actual-size placement template or sew-out validation.')
    ...
    try:
        ...
        for title,image in zip(('Needle-penetration review','Sewn-thread-length review'),maps):
